...
#+BEGIN_SRC emacs-lisp :results none
  ...
  (load (concat load-custom-path "/ob-php.el"))
...
#+begin_src emacs-lisp :results none
  (if (plist-get document-production :latex)
      (progn
        (use-package latex
          :ensure auctex
          :init (progn
...
PDF Tools
#+begin_src emacs-lisp :results none
  (if (plist-get document-production :pdf-tools)
      (progn
        (use-package tablist
          :ensure t)
...
#+name: plantuml-setup
 #+begin_src emacs-lisp :var plantuml_dir="~/.emacs.d/plantuml.jar" :results none
  (if (plist-get document-production :plantuml)
