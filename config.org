...
#+BEGIN_SRC emacs-lisp :results none
  ...
  ;; Which key mode (built-in since emacs 30)
  (which-key-mode t)
#+end_SRC
...
Corfu is a minimalist completion framework. It brings up a list at-point when auto complete options are available. Can use tab to cycle through the list. Note that =M-/= is the default auto complete behaviour (which doesn't display the list of options)
#+BEGIN_SRC emacs-lisp :results none
  (use-package company
    :ensure t
...
Language server client (LSP mode) (must have the associated LSP server installed for each language)
#+begin_src emacs-lisp :results none
...
#+BEGIN_SRC emacs-lisp :results none
  ...
  (use-package olivetti
    :ensure t
    :config
    (add-hook 'org-mode-hook (lambda () (olivetti-set-width 90)))
...
*** TODO
- [ ] Citation manager
- jupyternotebooks / org mode integration
